...
import { queueTransform, getJobStatus, analyzeDesign, analyzeClutter, declutter, getStyles, getLightingOptions, getTimeOptions } from '../controllers/transform.controller';
import { authenticate } from '../middleware/auth.middleware';

const router = Router();

router.post('/transform', authenticate, queueTransform);
router.get('/jobs/:jobId', authenticate, getJobStatus);
router.post('/analyze', authenticate, analyzeDesign);
router.post('/analyze-clutter', authenticate, analyzeClutter);
router.post('/declutter', authenticate, declutter);

router.get('/styles', getStyles);
router.get('/lighting', getLightingOptions);
router.get('/time-of-day', getTimeOptions);
